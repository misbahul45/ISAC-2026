...
test('can get team data for form', function (): void {
    $this->team->update([
        'name' => 'Tim Sains', 'phone' => '08123456789', 'school_name' => 'SMAN 1 Jakarta',
        'school_province' => 'DKI Jakarta', 'school_city' => 'Jakarta Selatan',
...
test('can update team data with registration progress', function (): void {
    ...
    $this->withToken($this->token)
        ->patchJson('/api/registrations/me/team', [
            'name' => 'Tim Baru', 'phone' => '081111111', 'school_name' => 'SMA Baru',
            'school_province' => 'Jawa Barat', 'school_city' => 'Bandung',
            'school_address' => 'Jl. Baru No. 1',
        ])
        ->assertOk()
        ->assertJsonPath('data.context.team.name', 'Tim Baru')
        ->assertJsonPath('data.context.team.schoolCity', 'Bandung')
